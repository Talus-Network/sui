...
pub struct Indexer<S: Store> {
    ...
    /// The checkpoint lowerbound derived from the committer watermarks of pipelines added to the
    /// indexer. Pipelines without watermarks default to 0, unless overridden by
    /// [Self::first_checkpoint]. When the indexer runs, it will start ingesting from this point.
...
impl<S: Store> Indexer<S> {
    ...
    pub async fn new(
        ...
        let task = task.map(|t| Task {
            task: t,
            reader_interval_ms: reader_interval_ms.expect("reader_interval_ms is required"),
        });
    ...
    /// Start ingesting checkpoints from `first_checkpoint_from_watermark`. Individual pipelines
    ...
    pub async fn run(mut self) -> Result<JoinHandle<()>> {
        ...
        info!(self.first_checkpoint_from_watermark, last_checkpoint = ?self.last_checkpoint, "Ingestion range");

        let broadcaster_handle = self
            .ingestion_service
            .run(
                self.first_checkpoint_from_watermark..=last_checkpoint,
                self.next_sequential_checkpoint,
            )
            .await
    ...
    async fn add_pipeline<P: Processor + 'static>(&mut self) -> Result<Option<u64>> {
        ...
        self.first_checkpoint_from_watermark =
            next_checkpoint.min(self.first_checkpoint_from_watermark);
...
mod tests {
    ...
    #[tokio::test]
    async fn test_first_checkpoint_from_watermark_all_pipelines_have_watermarks() {
        ...
        assert_eq!(indexer.first_checkpoint_from_watermark, 2);
    }

    /// first_checkpoint_from_watermark is 0 when at least one pipeline has no watermark.
    #[tokio::test]
    async fn test_first_checkpoint_from_watermark_not_all_pipelines_have_watermarks() {
        let cancel = CancellationToken::new();
        let registry = Registry::new();
        let store = MockStore::default();
    ...
    /// first_checkpoint_from_watermark is 1 when smallest committer watermark is 0.
    #[tokio::test]
    async fn test_first_checkpoint_from_watermark_smallest_is_0() {
        let cancel = CancellationToken::new();
        let registry = Registry::new();
        let store = MockStore::default();
    ...
    /// first_checkpoint_from_watermark is first_checkpoint when at least one pipeline has no
    /// watermark, and first_checkpoint is smallest.
    #[tokio::test]
    async fn test_first_checkpoint_from_watermark_first_checkpoint_and_no_watermark() {
        let cancel = CancellationToken::new();
        let registry = Registry::new();
        let store = MockStore::default();
    ...
    /// first_checkpoint_from_watermark is smallest among existing watermarks + 1 if
    /// first_checkpoint but all pipelines have watermarks (ignores first_checkpoint).
    #[tokio::test]
    async fn test_first_checkpoint_from_watermark_ignore_first_checkpoint() {
        let cancel = CancellationToken::new();
        let registry = Registry::new();
        let store = MockStore::default();
    ...
    #[tokio::test]
    async fn test_first_checkpoint_from_watermark_large_first_checkpoint() {
        let cancel = CancellationToken::new();
        let registry = Registry::new();
        let store = MockStore::default();
    ...
    async fn test_indexer_ingestion_existing_watermarks_no_first_checkpoint() {
        ...
        // Ingest from checkpoint 6 to 29 inclusive = 24 checkpoints.
        assert_eq!(metrics.total_ingested_checkpoints.get(), 24);
        assert_eq!(
            metrics
    ...
    async fn test_indexer_ingestion_use_first_checkpoint() {
        ...
        assert_eq!(metrics.total_ingested_checkpoints.get(), 20);
        assert_eq!(
            metrics
                .total_watermarks_out_of_order
                .get_metric_with_label_values(&[A::NAME])
